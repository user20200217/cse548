...
t_min = 1 # time limit between packets; unit is ns
last_seen = dict() # ip: time last seen
banned_ip = set() # banned ips
n_max = 5 # limit of ips per mac
mac_ip = dict() # mac: [ips]
banned_mac = set() # banned macs
...
class Firewall (EventMixin):
	...
	def _handle_PacketIn(self, event):
		...
		if (ip is not None) and (eth is not None):
			# part 1: defense against static ip
			source_ip = ip.srcip.toStr()
			if last_seen.get(source_ip,None)==None:
				last_seen[source_ip] = time()
			else:
				now = time()
				if (now-last_seen[source_ip]<t_min) and (source_ip not in banned_ip): # not enough time between packets
					# ban mac
					message = of.ofp_flow_mod()
					match = of.ofp_match()
					match.nw_src = IPAddr(source_ip)
					match.nw_dst = None
					match.dl_type = pkt.ethernet.IP_TYPE
					message.priority = 65535
					message.match = match			
					event.connection.send(message)
					banned_ip.add(source_ip)
					print('banned ip '+ source_ip)
				last_seen[source_ip] = now
			# part 2: defense against random ip
			source_mac = eth.src.toStr()
			if mac_ip.get(source_mac,None)==None:
				mac_ip[source_mac] = set()
			mac_ip[source_mac].add(source_ip)
			if (len(mac_ip[source_mac])>n_max) and (source_mac not in banned_mac): # same mac has too many ips
				# ban mac
				message = of.ofp_flow_mod()
				match = of.ofp_match()
				match.dl_src = EthAddr(source_mac)
				match.dl_dst = None
				message.priority = 65535
				message.match = match			
				event.connection.send(message)
				banned_mac.add(source_mac)
				print('banned mac '+ source_mac)
